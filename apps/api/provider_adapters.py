        ...
        if capability=="place_call":
            ...
            try:
                r=httpx.post(f"https://api.twilio.com/2010-04-01/Accounts/{sid}/Calls.json",
                             auth=(sid,token),data={"From":sender,"To":to,"Url":twiml_url,**({"StatusCallback":payload.get("status_callback") or os.getenv("LUMA_VOICE_STATUS_CALLBACK_URL"),"StatusCallbackEvent":["initiated","ringing","answered","completed"],"TimeLimit":int(payload.get("time_limit_seconds") or os.getenv("LUMA_VOICE_MAX_DURATION_SECONDS","1800"))} if (payload.get("status_callback") or os.getenv("LUMA_VOICE_STATUS_CALLBACK_URL")) else {"TimeLimit":int(payload.get("time_limit_seconds") or os.getenv("LUMA_VOICE_MAX_DURATION_SECONDS","1800"))})},timeout=20)
