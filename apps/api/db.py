...
def ensure_delivery_schema():
    ...
    CREATE TABLE IF NOT EXISTS evidence_claims (
      ...
      research_report_id UUID REFERENCES research_reports(id) ON DELETE SET NULL, opportunity_id UUID REFERENCES opportunities(id) ON DELETE SET NULL,
      claim TEXT NOT NULL, evidence_strength NUMERIC(5,2) NOT NULL DEFAULT 0, alternatives JSONB NOT NULL DEFAULT '[]',
    ...
    CREATE TABLE IF NOT EXISTS agent_action_receipts (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(), agent_run_id UUID REFERENCES agent_runs(id) ON DELETE SET NULL,
      actor_type TEXT NOT NULL, actor_id UUID, action TEXT NOT NULL, decision TEXT NOT NULL, risk TEXT NOT NULL,
      reason TEXT NOT NULL, estimated_cost NUMERIC(12,6) NOT NULL DEFAULT 0, previous_hash TEXT, receipt_hash TEXT NOT NULL,
      entity_type TEXT, entity_id UUID, metadata JSONB NOT NULL DEFAULT '{}', created_at TIMESTAMPTZ NOT NULL DEFAULT now()
    );
    ...
    CREATE TABLE IF NOT EXISTS workflow_checkpoints (
      ...
      step TEXT NOT NULL, state JSONB NOT NULL DEFAULT '{}', checkpoint_hash TEXT, created_at TIMESTAMPTZ NOT NULL DEFAULT now()
    );
    ...
    CREATE TABLE IF NOT EXISTS service_blueprint_versions (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(), service_id UUID REFERENCES services(id) ON DELETE CASCADE,
      version INT NOT NULL DEFAULT 1, blueprint JSONB NOT NULL DEFAULT '{}', active BOOLEAN NOT NULL DEFAULT true,
      created_at TIMESTAMPTZ NOT NULL DEFAULT now(), UNIQUE(service_id, version)
    );
    CREATE TABLE IF NOT EXISTS agent_budget_ledger (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(), agent_name TEXT NOT NULL,
      project_id UUID REFERENCES projects(id) ON DELETE SET NULL, workflow_run_id UUID REFERENCES workflow_runs(id) ON DELETE SET NULL,
      agent_run_id UUID REFERENCES agent_runs(id) ON DELETE SET NULL, amount NUMERIC(12,6) NOT NULL DEFAULT 0,
      category TEXT NOT NULL DEFAULT 'model', occurred_at TIMESTAMPTZ NOT NULL DEFAULT now()
    );
    ...
    CREATE TABLE IF NOT EXISTS client_metric_snapshots (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(), client_id UUID REFERENCES clients(id) ON DELETE CASCADE,
      project_id UUID REFERENCES projects(id) ON DELETE CASCADE, metric_name TEXT NOT NULL, value NUMERIC(14,4),
      unit TEXT, source TEXT, client_confirmed BOOLEAN NOT NULL DEFAULT false, captured_at TIMESTAMPTZ NOT NULL DEFAULT now()
    );
